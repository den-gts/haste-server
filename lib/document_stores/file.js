...
FileDocumentStore.prototype.get = function(key, callback, skipExpire) {
  ...
  fs.readFile(fn, 'utf8', function(err, data) {
    if (err) {
      ...
      callback(data);
      if (_this.expire && !skipExpire) {
        winston.warn('file store cannot get key');
      }
    }
  });
};

FileDocumentStore.prototype.getConnected = function(callback) {
  return fs.access(this.basePath, fs.constants.F_OK,function (err){ callback(err === null) });
};
